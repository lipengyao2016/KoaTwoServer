...
const log = require("../common/log").getLogger();
...
exports.handlerError = function(rtx,e){
    console.error(e);
    log.error(e);
    e = this.isDBError(e);
    this.errorReturn(rtx, e.status, e);
}
...
exports.errorReturn = function(rtx, status, error){
    let body = {
    ...
    rtx.set({'Content-Type': this.retContentType});
    rtx.status = status?status:500;
    rtx.body = body;
    let logString = `status: ${status} name: ${body.name} code: ${body.code} message: ${body.message} description: ${body.description}`;
